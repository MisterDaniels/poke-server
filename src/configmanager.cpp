...
bool ConfigManager::load()
{
	...
	decimal[MONSTERLEVEL_BONUSLOOT] = getGlobalBoolean(L, "monsterLevelLoot", 0.0);
...
double ConfigManager::getGlobalDouble(lua_State* L, const char* identifier, cons double defaultValue)
{
	lua_getglobal(L, identifier);
	if (!lua_isnumber(L, -1)) {
		return defaultValue;
